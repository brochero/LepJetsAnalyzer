...
void Plotter(TString plots="2btag", bool LogScale=false, TString cat = "ttbb", TString nSyst = "btagcfI"){
  ...
  ****************/ 
  ...
  ttbar_Nom  = loadhistograms(plots, files + "_ttbar_LepJetsPowhegPythiaTranche3" + cat);
  ttbar_Up   = loadhistograms(plots, files + "_ttbar_LepJetsPowhegPythiaTranche3" + cat + "_SYS_" + nSyst + "_Up");
  ttbar_Down = loadhistograms(plots, files + "_ttbar_LepJetsPowhegPythiaTranche3" + cat + "_SYS_" + nSyst + "_Down");
  ...
  ****************/ 
  ...
  for(unsigned int h=0; h<ttbar_Nom.size(); h++){
    for(int ch=0; ch<3; ch++){
      ...
      ttbar_Nom[h].hist[ch]->SetTitle("");

      // Produce enough vertical space for the legend 
      float MaxHisto;
      if(LogScale) MaxHisto = 100.0;
      else MaxHisto = 1.7;
      float maxh = ttbar_Nom[h].hist[ch]->GetMaximum();
      if(maxh < ttbar_Nom[h].hist[ch]->GetMaximum()) maxh = ttbar_Nom[h].hist[ch]->GetMaximum();
      ...
      ***********************/    
      ...
      RatioDown = (TH1D*)ttbar_Nom[h].hist[ch]->Clone();
      RatioUp   = (TH1D*)ttbar_Nom[h].hist[ch]->Clone();
      RatioDown->Divide(ttbar_Down[h].hist[ch]);
      RatioUp  ->Divide(ttbar_Up[h].hist[ch]);
      ...
      RatioUp->SetLineColor(col_Up);
      RatioUp->SetLineWidth(1);
      RatioUp->SetTitle("");
      
      RatioDown->SetLineColor(col_Down);
      ...
      ***********************/    
      ...
      dirfigname_pdf = dirnameIn + "figures_" + fl + "/ttbbNomUpDown/pdf_" + nSyst + "_" + cat + dirfigname_log + "/";
      // make a dir if it does not exist!!
      gSystem->mkdir(dirfigname_pdf,       kTRUE);
      histocanvas->SaveAs(dirfigname_pdf + ttbar_Nom[h].hist[ch]->GetName() + ".pdf");      
      // clear Canvas
      histocanvas->Clear();    
...
void PlotsComSystVar(TString SystVar = "btagcfI"){
  
  Plotter("2btag", false, "ttbb", SystVar);
  Plotter("2btag", false, "ttbj", SystVar);
  Plotter("2btag", false, "ttcc", SystVar);
  Plotter("2btag", false, "ttLF", SystVar);
  Plotter("2btag", false, "tt",   SystVar);
